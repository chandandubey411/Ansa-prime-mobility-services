...
export default function FeaturedCars() {
  ...
  return (
    <section className="py-28 relative overflow-hidden" style={{ background: 'linear-gradient(180deg, #111 0%, #0a0a0a 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-16">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.35 }}
            className="font-body text-sm text-white/40 mt-4 max-w-md mx-auto leading-relaxed">
        ...
        <div className="relative">
          ...
          <div className="flex justify-end gap-3 mb-5">
            <button ref={prevRef}
              className="w-11 h-11 rounded-full border border-gold/30 flex items-center justify-center text-gold hover:bg-gold hover:text-dark-DEFAULT transition-all duration-300">
              <HiArrowLeft />
            </button>
            <button ref={nextRef}
              className="w-11 h-11 rounded-full border border-gold/30 flex items-center justify-center text-gold hover:bg-gold hover:text-dark-DEFAULT transition-all duration-300">
            ...
            {featuredCars.map((car) => (
              <SwiperSlide key={car.name}>
                ...
                >
                  ...
                  {/* Info */}
                  <div className="p-5">
                    <h3 className="font-heading text-base font-bold text-white mb-1 group-hover:text-gold transition-colors duration-300">
                      {car.name}
                    </h3>
                    <p className="font-body text-[11px] text-white/35 mb-4 tracking-wider uppercase">{car.category}</p>

                    <div className="flex flex-wrap gap-x-4 gap-y-1 mb-5">
                      <span className="flex items-center gap-1.5 text-[12px] text-white/45 font-body">
